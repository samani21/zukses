...
// --- ICONS ---
const Edit2Icon: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const Camera: FC<{ size?: number; className?: string, strokeWidth?: number }> = ({ size = 24, className = '', strokeWidth = 2 }) => (
...
const X: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const Video: FC<{ size?: number; className?: string, strokeWidth?: number }> = ({ size = 24, className = '', strokeWidth = 2 }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" className={className}><path d="m22 8-6 4 6 4V8z"></path><rect x="2" y="6" width="14" height="12" rx="2" ry="2"></rect></svg>
);
const Trash2: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
);
const Info: FC<{ size?: number; className?: string }> = ({ size = 16, className = '' }) => (
...
const Eye: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const Lightbulb: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const ShoppingBag: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const MessageCircleIcon: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const ShoppingCartIcon: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const ChevronLeft: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const ChevronRight: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const Search: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const Plus: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const Move: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
...
const LayersIcon: FC<{ size?: number; className?: string }> = ({ size = 24 }) => (
...
const Calendar: FC<{ size?: number; className?: string }> = ({ size = 24, className = '' }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
);
const CheckCircle: FC<{ size?: number; className?: string }> = ({ size = 16, className = '' }) => (
...
const CategorySelector: FC<{ onSelectCategory: (name: string) => void; initialCategory: string; categories: Category[]; isLoading: boolean; error: string | null; setIdCategorie: (id: number) => void; handleConfirmCategory: () => void; onClose: () => void }> = ({ onSelectCategory, categories, setIdCategorie, handleConfirmCategory, onClose }) => {
    ...
    const lastSelected = path.length > 0 ? path[path.length - 1] : null;
    const finalSelection = lastSelected && (!lastSelected.children || lastSelected.children.length === 0);

    return (
        <div className="flex flex-col h-[60vh]">
            <div className="p-4 border-b">
                <h3 className="text-lg font-semibold text-gray-800">Ubah Kategori</h3>
                <div className="relative mt-2">
                    <input type="text" placeholder="Masukkan min. 1 karakter" className="w-full p-2 pl-8 border rounded-md" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                    <Search size={16} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
                </div>
            </div>
            <div className="flex-grow flex overflow-x-auto border-b">
                {columns.map((col, level) => (
                    <div key={level} className="w-60 border-r flex-shrink-0">
                        {col.items.filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase())).map(item => (
                            <button key={item.id} onClick={() => handleSelect(item, level)} className={`w-full text-left p-3 text-sm flex justify-between items-center hover:bg-orange-50 ${path[level]?.id === item.id ? 'bg-orange-100 text-orange-600 font-semibold' : ''}`}>
                                <span>{item.name}</span>
            ...
            <div className="p-4 flex justify-between items-center bg-gray-50">
                <div className="text-sm overflow-hidden whitespace-nowrap text-ellipsis">
                    ...
                    <span className="font-semibold text-gray-800">{path.map(p => p.name).join(' > ')}</span>
                </div>
                <div className="flex gap-2 flex-shrink-0">
                    <button onClick={onClose} className="px-4 py-2 border rounded-md text-gray-700">Batal</button>
                    <button onClick={handleConfirmCategory} disabled={!finalSelection} className="px-4 py-2 bg-orange-500 text-white rounded-md disabled:bg-orange-300 disabled:cursor-not-allowed">Konfirmasi</button>
                </div>
            </div>
        </div>
    );
...
const AddProductPage = () => {
    ...
    useEffect(() => {
        async function fetchCategories() {
            ...
            try {
                const res = await Get<Response<Category[]>>('zukses', 'category/show');
    ...
    useEffect(() => {
        const handleScroll = () => {
            const scrollPosition = window.scrollY + 80; // Adjusted offset for sticky nav
    ...
    const renderPreviewContent = () => (
        <>
            <div className="p-3">
                <div className={`relative bg-gray-200 aspect-square rounded-md mb-3 flex items-center justify-center overflow-hidden`}>
    ...
    return (
        <div className="bg-gray-100 min-h-screen font-sans pb-32">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-4 items-center mt-1">
                                                {productPhotos.map((photo, index) => (
                                                    <div key={photo.preview} className="relative w-24 h-24 flex-shrink-0 group">
                    ...
                    <div className="lg:col-span-6 space-y-4">
                        <div className="bg-white rounded-lg shadow-sm">
                            ...
                            >
                                {renderSection("Informasi Produk", (
                                    <>
                                        <div onMouseEnter={() => setFocusedField('photo')} onMouseLeave={() => setFocusedField(null)}>
                                        ...
                                        <div onFocus={() => setFocusedField('productName')} onBlur={() => setFocusedField(null)}>
                                            ...
                                            <div className="relative flex-grow">
                                                <input type="text" value={productName} maxLength={255} onChange={(e) => setProductName(e.target.value)} placeholder="Nama Merek + Tipe Produk (Bahan, Warna, Ukuran, Variasi)" className={`w-full p-2 border rounded-md shadow-sm pr-16 border-gray-300`} />
                                                <span className="absolute inset-y-0 right-3 flex items-center text-sm text-gray-400">{productName.length}/255</span>
                                            </div>
                                        </div>
                                        <div onMouseEnter={() => setFocusedField('category')} onMouseLeave={() => setFocusedField(null)}>
                                        ...
                                        <div onFocus={() => setFocusedField('description')} onBlur={() => setFocusedField(null)}>
