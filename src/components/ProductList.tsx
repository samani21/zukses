        ...
        <div className="container mx-auto p-4 md:px-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
                {filteredProducts?.map(product => (
                    <a key={product.id} className="bg-white rounded-lg overflow-hidden group" onClick={() => {
                        window.location.href = `/${product.name.toLowerCase().replace(/\s+/g, '-')}`
                        ...
                        <div className="p-4 pl-0">
                            ...
                            <p className="text-xs text-gray-500 mt-2">Indonesia</p>
                            <div className="flex items-center mt-2 text-xs text-gray-500">
                                <StarIcon className="w-4 h-4 text-yellow-400 mr-1" />
                                <span>{product.rating || 5} | Terjual {product.sold || "1jt"}+</span>
                            </div>
                        </div>
                    </a>
                ))}
