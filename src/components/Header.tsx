...
// --- Komponen SearchSuggestions untuk Desktop (Tidak ada perubahan) ---
...
// --- [PERUBAIKAN 1] ---
// Komponen suggestion BARU, khusus untuk mobile, dengan layout list biasa.
const MobileSearchSuggestions = ({ suggestions, onSuggestionClick }: { suggestions: Suggestion[], onSuggestionClick: (suggestion: Suggestion) => void }) => {
    ...
    return (
        // Ini adalah list biasa, bukan 'absolute'
        <div>
            <ul>
...
// --- [PERUBAIKAN 2] ---
// Komponen MobileSearch diperbarui untuk menggunakan MobileSearchSuggestions
const MobileSearch = ({ onClose, suggestions, searchHistory, onSearchSubmit }: { onClose: () => void, suggestions: Suggestion[], searchHistory: Suggestion[], onSearchSubmit: (term: string) => void }) => {
    ...
    // Logika filter dipindahkan ke sini agar lebih jelas
    const suggestionsToShow = searchTerm.trim() === ''
        ? searchHistory
        : suggestions.filter(s => s.text.toLowerCase().includes(searchTerm.toLowerCase()));
...
const Header = () => {
    // Sisa komponen Header tidak ada perubahan logika, hanya integrasi komponen di atas
                                        ...
                                        <div className="w-full">
                                            <div className='relative flex-grow border border-[0.5px] border-[#555555] rounded-[5px]'>
                                                <input type="text" placeholder="Cari di Zukses" value={searchTerm}
                                                    onFocus={() => { setIsSearchFocused(true); setDropdownMode('history'); }}
                                                    onChange={(e) => { const newTerm = e.target.value; setSearchTerm(newTerm); setDropdownMode(newTerm.trim() === '' ? 'history' : 'suggestions'); }}
                                                    onKeyDown={(e) => { if (e.key === 'Enter') handleSearch(); }}
                                        ...
                                        <div className='flex items-center w-[400px] justify-between gap-4'>
                                            <div className='flex items-center jutify-left gap-3'>
                                                ...
                                                <div className="relative inline-block">
                                                    ...
                                                    <span className="absolute bottom-6 left-6 w-2.5 h-2.5 bg-red-500 rounded-full border border-white"></span>
                                                </div>
                                                <div className="relative inline-block">
                                                    <button className="p-2 rounded-md pr-0">
                                                ...
                                                <div className="relative inline-block">
                                                    ...
                                                    <span className="absolute bottom-5 left-5 bg-red-500 px-1 text-[10px] rounded-[5px] border border-white">123</span>
                                                </div>
                                            </div>
                                            <button className="p-2 rounded-md bg-[#4A52B2] text-[13px] w-[90px] font-semibold text-white" title="Toko Saya">
                                                Akun Saya
                                            </button>
                                        </div>
                                    </div>
